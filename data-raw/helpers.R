...
#' @description
#' Converts any recognized missing values in the input dataframe to NA.
#' If the verbose parameter is not explicitly provided, the function defaults to using the
#' value from the environment variable GEN_verbose.
#' This allows control over the message output without modifying the function call directly.
#' @param df The input dataframe.
#' @param char_as_factor Logical; if TRUE, character variables will automatically be converted
#'    to factors. (Default: TRUE)
#' @param verbose Logical; if TRUE, variables without a defined label will be shown during execution.
#'   If missing, the function will use the value of the environment variable GEN_verbose.
#' @returns A dataframe with missing values converted to NA.
#' @noRd
...
#' @param custom_description Optional custom description for the dataset
#' @param custom_source Optional custom source information for the dataset
...
roxygen2_data <- function(
  ...
  if (!is.null(custom_description)) {
    descr <- paste0("#' @description ", custom_description)
  } else {
    descr <- paste0(
      "#' @description ",
      df_name,
      " modified from pharmaverseadam"
    )
  }
