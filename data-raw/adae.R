...
gen_adae <- function(seed = 123) {
  ...
  gen <- gen %>%
    mutate(
      ...
      AELLT = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034")  & AESEQ == 1, "DYSFUNCTIONAL UTERINE BLEEDING", AELLT),
      AEDECOD = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034")  & AESEQ == 1, "ABNORMAL UTERINE BLEEDING", AEDECOD),
      AEHLT = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034")  & AESEQ == 1, "MENSTRUATION AND UTERINE BLEEDING NEC", AEHLT),
      AEHLGT = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034") & AESEQ == 1 , "MENSTRUAL CYCLE AND UTERINE BLEEDING DISORDERS", AEHLGT),
      AEBODSYS = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034") & AESEQ == 1, "REPRODUCTIVE SYSTEM AND BREAST DISORDERS", AEBODSYS),
      AESOC = ifelse(USUBJID %in% c("01-701-1015", "01-701-1034")  & AESEQ == 1, "REPRODUCTIVE SYSTEM AND BREAST DISORDERS", AESOC)
    )
  ...
  gen <- gen %>%
    mutate(
      ...
      AEHLT = ifelse(USUBJID %in% c("01-701-1023", "01-701-1028") & AESEQ == 1, "ERECTION AND EJACULATION CONDITIONS AND DISORDERS", AEHLT),
      AEHLGT = ifelse(USUBJID %in% c("01-701-1023", "01-701-1028") & AESEQ == 1, "SEXUAL FUNCTION AND FERTILITY DISORDERS", AEHLGT),
      AEBODSYS = ifelse(USUBJID %in% c("01-701-1023", "01-701-1028") & AESEQ == 1, "REPRODUCTIVE SYSTEM AND BREAST DISORDERS", AEBODSYS),
      AESOC = ifelse(USUBJID %in% c("01-701-1023", "01-701-1028") & AESEQ == 1, "REPRODUCTIVE SYSTEM AND BREAST DISORDERS", AESOC)
    )

  # c.  First records Placebo and Treatment for Female Broad
  gen <- gen %>%
    mutate(
      AETERM = ifelse(USUBJID %in% c("01-701-1363", "01-701-111") & AESEQ == 1, "BLEEDING ANOVULATORY", AETERM),
      AELLT = ifelse(USUBJID %in% c("01-701-1363", "01-701-111")  & AESEQ == 1, "ANOVULAR DYSFUNCTIONAL UTERINE BLEEDING", AELLT),
      AEDECOD = ifelse(USUBJID %in% c("01-701-1363", "01-701-111")  & AESEQ == 1, "BLEEDING ANOVULATORY", AEDECOD),
      AEHLT = ifelse(USUBJID %in% c("01-701-1363", "01-701-111")  & AESEQ == 1, "FEMALE GONADAL FUNCTION DISORDERS", AEHLT),
      AEHLGT = ifelse(USUBJID %in% c("01-701-1363", "01-701-111") & AESEQ == 1 , "ENDOCRINE DISORDERS OF GONADAL FUNCTION", AEHLGT),
      AEBODSYS = ifelse(USUBJID %in% c("01-701-1363", "01-701-111") & AESEQ == 1, "ENDOCRINE DISORDERS", AEBODSYS),
      AESOC = ifelse(USUBJID %in% c("01-701-1363", "01-701-111")  & AESEQ == 1, "ENDOCRINE DISORDERS", AESOC)
    )

  # d.  First records Placebo and Treatment for Male Broad
  gen <- gen %>%
    mutate(
      AETERM = ifelse(USUBJID %in%c("01-701-1392", "01-701-1097") & AESEQ == 1, "DISTURBANCE IN SEXUAL AROUSAL", AETERM),
      AELLT = ifelse(USUBJID %in%c("01-701-1392", "01-701-1097") & AESEQ == 1, "SEXUAL AROUSAL DECREASED", AELLT),
      AEDECOD = ifelse(USUBJID %in%c("01-701-1392", "01-701-1097") & AESEQ == 1, "DISTURBANCE IN SEXUAL AROUSAL", AEDECOD),
      AEHLT = ifelse(USUBJID %in%c("01-701-1392", "01-701-1097") & AESEQ == 1, "SEXUAL AROUSAL DISORDERS", AEHLT),
      AEHLGT = ifelse(USUBJID %in%c("01-701-1392", "01-701-1097") & AESEQ == 1, "SEXUAL DYSFUNCTIONS, DISTURBANCES AND GENDER IDENTITY DISORDERS", AEHLGT),
