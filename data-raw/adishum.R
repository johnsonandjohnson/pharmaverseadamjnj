...
add_adishum_col_funcs$param_n_paramcd <- function(main_tbl) {
  ...
  valid_visitnums <- add_adishum_col_funcs$sample_visits() |>
    dplyr::pull(VISITNUM)
  ...
  # subject summary has no visit context - visitnum set to 0
...
add_adishum_col_funcs$dtl <- function(
  ...
  n <- nrow(subj)
  probs <- bins$pct / sum(bins$pct)
  bin_id <- sample(seq_len(nrow(bins)), size = n, replace = TRUE, prob = probs)

  base <- stats::runif(n, bins$lower[bin_id], bins$upper[bin_id])
  vals <- base + stats::runif(n, noise)
  vals <- pmax(bins$lower[bin_id], pmin(bins$upper[bin_id], vals))

  subj$DTL <- round(vals, digits)

  dplyr::left_join(main_tbl, subj, by = "USUBJID")
}

# add ADATRES column - subject-level: POSITIVE for ADATRE, NEGATIVE for ADANTRE, propagated to all rows
add_adishum_col_funcs$adatres <- function(main_tbl) {
  main_tbl |>
    ...
    dplyr::group_by(USUBJID) |>
    tidyr::fill(ADATRES, .direction = "updown") |>
    dplyr::ungroup()
}

# add NABSTAT column - subject-level: POSITIVE for NABPOS, NEGATIVE for NABNEG, propagated to all rows
add_adishum_col_funcs$nabstat <- function(main_tbl) {
  main_tbl |>
    ...
    dplyr::group_by(USUBJID) |>
    tidyr::fill(NABSTAT, .direction = "updown") |>
    dplyr::ungroup()
}
...
add_adishum_col_funcs$dataset_tests <- function(main_tbl) {
  ...
  collec_visitnums <- add_adishum_col_funcs$sample_visits() |>
    dplyr::pull(VISITNUM)
