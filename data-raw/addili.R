...
gen_addili <- function(seed = 123) {
  ...
  addili_prep <- addili_merged |>
    mutate(
      ...
      CRIT2 = if_else(PARCAT1 == "ALT or AST", ">= 5x ULN", NA_character_),
      CRIT2FL = case_when(
        PARCAT1 == "ALT or AST" & R2ANRHI >= 5 ~ "Y",
        PARCAT1 == "ALT or AST" & !is.na(R2ANRHI) ~ "N",
        TRUE ~ NA_character_
      ),
      CRIT3 = if_else(PARCAT1 == "ALT or AST", ">= 8x ULN", NA_character_),
      CRIT3FL = case_when(
  ...
  additional_labels <- list(
    ...
    R2ANRHI  = "Ratio to Analysis Normal Range Upper Limit",
